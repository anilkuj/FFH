...
export function renderPlanner(container, state, actions) {
    ...
    // Benchmark: 5.80 base XP/player average (63.8 base starters XP + ~6.7 captaincy bonus = 70.5 GW total) = 100 rating.
    // Distribution: Average squad (~55 total GW xP) → ~78 rating | Strong template (~63 total xP) → ~89 | Elite AI (~69 total xP) → ~96 | World #1 Pace (72+ total xP) → 100
    ...
    const ratingScore = Math.min(100, Math.round((averagePlayerXP / 5.80) * 100));
