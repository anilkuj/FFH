...
export function openPlayerDetailModal(playerId, type, starters, bench, state, actions, triggerSwapCallback) {
    ...
    let currentSeasonStarts = 0;
    let currentSeasonAvgMins = 0;
    let currentSeasonTotalMins = 0;
    ...
    if (hasPlayedMatches) {
        playedPredictions.forEach(pr => {
            let pts = pr.actualPts;
            if (state && state.livePoints && state.livePoints[pr.gw] && state.livePoints[pr.gw][player.id] !== undefined) {
                pts = state.livePoints[pr.gw][player.id];
            }
            let mins = pr.mins;
            if (mins === undefined) {
                if (pts > 0) {
                    mins = Math.max(60, Math.round(lastSeasonAvgMins || 75));
                } else if (lastSeasonStarts > 15) {
                    mins = Math.round(lastSeasonAvgMins || 75);
                } else {
                    mins = 0;
                }
            }
            currentSeasonTotalMins += mins;
            if (mins > 45 || pts > 0 || lastSeasonStarts > 15) {
                currentSeasonStarts++;
            }
        });
        currentSeasonAvgMins = Math.round(currentSeasonTotalMins / currentSeasonPlayedGws);
    } else {
        currentSeasonStarts = lastSeasonStarts;
        currentSeasonAvgMins = lastSeasonAvgMins;
    }
                ...
                <div>
                    ...
                    <div class="player-detail-stats-grid" style="padding: 0; margin-bottom: 0; grid-template-columns: repeat(3, 1fr); gap: 10px; width: 100%;">
                        ...
                        <div class="detail-stat-box" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 10px;">
                            ...
                            <span class="detail-stat-lbl" style="font-size: 9px; margin-top: 6px;">Availability</span>
                        </div>
                    </div>
                </div>
            ...
            <div class="detail-right-column" style="display: flex; flex-direction: column; gap: 16px; min-width: 0; border-left: 1px solid var(--border-color); padding-left: 20px;">
                ...
                <div>
                    ...
                    <div id="optaStatsCurrentView" class="player-detail-stats-grid" style="margin: 0; padding: 0; display: grid; grid-template-columns: repeat(4, 1fr); gap: 6px;">
                        ...
                        <div class="detail-stat-box" style="padding: 6px 8px; text-align: center;">
                            <span class="detail-stat-val" style="font-size: 13px; font-weight: 700;">${player.ownership.toFixed(1)}%</span>
                        ...
                        <div class="detail-stat-box" style="padding: 6px 8px; text-align: center;">
                            <span class="detail-stat-val" style="font-size: 13px; font-weight: 700;">${currentSeasonStarts}</span>
                            <span class="detail-stat-lbl" style="font-size: 9px; color: var(--text-muted); display: block;">${hasPlayedMatches ? 'Starts' : 'Exp Starts'}</span>
                        </div>
                        <div class="detail-stat-box" style="padding: 6px 8px; text-align: center;">
    ...
    actions.showModal(modalContent, () => {
        ...
        const candidateRowsData = replacementCandidates.map(comp => {
            const nextXp = getNGwXp(comp, state.currentGw, 1);
            const fiveXp = get5GwXp(comp, state.currentGw);
            const last5Pts = getLast5GamesActualPts(comp, state.currentGw, state);
            const fdrData = getNext5FixturesFDR(comp, state.currentGw);
            const fdrAvg = parseFloat(fdrData.avg);
        ...
        const renderCandidateRows = () => {
            candidateRowsData.sort((a, b) => {
                let valA = a.comp.name, valB = b.comp.name;
                if (sortCol === 'price') { valA = a.comp.price; valB = b.comp.price; }
                else if (sortCol === 'xp5') { valA = a.fiveXp; valB = b.fiveXp; }
                else if (sortCol === 'xp1') { valA = a.nextXp; valB = b.nextXp; }
                else if (sortCol === 'form') { valA = a.last5Pts; valB = b.last5Pts; }
                else if (sortCol === 'fdr') { valA = a.fdrAvg; valB = b.fdrAvg; }
                else if (sortCol === 'name') { valA = a.comp.name; valB = b.comp.name; }
        ...
        ths.forEach(th => {
            th.addEventListener('click', () => {
                ...
                ths.forEach(otherTh => {
                    ...
                    const arrowSpan = otherTh.querySelector('.sort-arrow');
                    if (arrowSpan) {
                        if (c === sortCol) {
                            arrowSpan.textContent = sortAsc ? '↑' : '↓';
                            arrowSpan.style.opacity = '1';
                            arrowSpan.style.color = 'var(--primary)';
                        } else {
                            arrowSpan.textContent = '↕';
                            arrowSpan.style.opacity = '0.4';
                            arrowSpan.style.color = 'inherit';
                        }
                    }
                });
