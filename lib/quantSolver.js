...
export function solveQuantStrategy(state, customParams = {}) {
    ...
    const prioritizeHomeGames = customParams.prioritizeHomeGames !== undefined ? customParams.prioritizeHomeGames : !!(state && state.prioritizeHomeGames);

    // Helper: Expected Points over Horizon with Fixture Difficulty Prioritization
    const getPlayerXpOverHorizon = (player, startGw, horizonLen) => {
        if (!player || !player.predictions) return 0;
        const factor = (typeof window !== 'undefined' && window.getPlayerMinutesFactor) ? window.getPlayerMinutesFactor(player) : 1.0;
        let total = 0;
        for (let gw = startGw; gw < startGw + horizonLen && gw <= 38; gw++) {
            ...
            if (pred) {
                ...
                let fdrBonus = 0;
                if (pred.diff !== undefined && pred.diff !== null) {
                    if (pred.diff <= 2) fdrBonus = (3 - pred.diff) * 0.4;
                    else if (pred.diff >= 4) fdrBonus = -(pred.diff - 3) * 0.4;
                }
                let homeBonus = 0;
                if (prioritizeHomeGames) {
                    const isHome = pred.loc === 'H' || pred.isHome === true;
                    if (isHome) {
                        const fdr = pred.diff !== undefined ? pred.diff : 3;
                        homeBonus = (6.0 - fdr) * 0.5;
                    }
                }
                total += ((raw + fdrBonus + homeBonus) * factor);
            }
        }
        return total;
    };
    ...
    // --------------------------------------------------------------------------
    // FPL RULE BREACH CHECK (Max 3 players per Premier League team)
    // --------------------------------------------------------------------------
    const teamCounts = {};
    ...
    const breachedTeams = Object.keys(teamCounts).filter(t => teamCounts[t] > 3);
    const hasRuleBreach = breachedTeams.length > 0;
    ...
    if (hasRuleBreach) {
        const breachedTeam = breachedTeams[0];
        const count = teamCounts[breachedTeam];
        const breachedPlayers = baselineAll.filter(p => p.team === breachedTeam).sort((a, b) => {
            return getPlayerXpOverHorizon(a, currentGw, horizon) - getPlayerXpOverHorizon(b, currentGw, horizon);
        });
        const playerToSell = breachedPlayers[0];

        const sellBudget = playerToSell.price + bank;
        ...
        ).sort((a, b) => getPlayerXpOverHorizon(b, currentGw, horizon) - getPlayerXpOverHorizon(a, currentGw, horizon));

        const replacement = candidates[0];

        actionType = 'RULE_BREACH';
        transfersOut = [playerToSell];
        transfersIn = replacement ? [replacement] : [];
        projectedEVGain = replacement ? (getPlayerXpOverHorizon(replacement, currentGw, horizon) - getPlayerXpOverHorizon(playerToSell, currentGw, horizon)) : 0;
        executiveRecommendation = `🚨 EMERGENCY RULE BREACH: You have ${count} ${breachedTeam} players selected (FPL limit is 3)! Transfer out ${playerToSell.name} (£${playerToSell.price.toFixed(1)}m) for ${replacement ? replacement.name + ' (£' + replacement.price.toFixed(1) + 'm)' : 'valid asset'} to restore compliance, or activate a Wildcard / Free Hit chip.`;
