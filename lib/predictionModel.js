...
// constant varied): 1.5 -> season pace ~3104; 1.3 -> ~2943. Target (user-specified, aiming for a
// raw no-captain/no-chips baseline sensibly below what a real #1-overall-rank manager's actual
// season total (2582, WITH captain doubling and chips already included) achieves): 2450-2500.
// The ceiling-to-season-total relationship isn't linear (only fixtures whose raw multiplier
// exceeds the ceiling are affected), so this value should be re-verified against real data after
// changing it, not assumed correct from extrapolation alone.
const PLAYER_ATTACK_MULTIPLIER_MIN = 0.5;
const PLAYER_ATTACK_MULTIPLIER_MAX = 1.1;
