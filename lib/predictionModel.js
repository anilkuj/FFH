...
export function computeBasePPG({ minutes, appearances, totalPoints, position, teamShort, price, isPromotedOrTransfer, manualOverridePPG }) {
    ...
    if (manualOverridePPG !== undefined && manualOverridePPG !== null) {
        ...
        const PRIOR_GAMES = 38; // Tuned shrinkage prior for all teams
...
const K_ATTACK_SPECIFIC = 0.002;
const K_ATTACK_OVERALL = 0.14;
...
const PLAYER_ATTACK_MULTIPLIER_MAX = 1.5;
 ...
 */
export function computeGwPrediction({ basePPG, position, xG90, xA90, saves90, mppg, starts, chanceOfPlaying, fixture, goalsConceded90, leagueAvgGoalsConceded90, setPieceDuty, dcPer90, teamShort, price }) {
    ...
    const isTarget = teamShort === 'ARS' || teamShort === 'MCI';
    if (isTarget && price > 5.5) {
        pts *= 0.88;
    }
