...
// MAX tightened 1.5 -> 1.3 after real-data calibration check: a representative strong 15-man
// squad's raw (no captain bonus, no chips) GW1-10 pace implied a ~2581-point season total --
// matching the actual 2025/26 #1-overall-rank manager's final total (2582), which already
// includes captain doubling and chip bonuses on top of raw squad output. A raw baseline landing
// at what a real winner achieves WITH those bonuses is too high; 1.3 reins in the compounding
// effect across a run of favorable fixtures (the dominant driver -- MIN's floor wasn't
// implicated, real complaint was specifically about elevated highs) while still meaningfully
// differentiating single games above the pre-Phase-3 blunt diff-based ceiling of 1.12.
const PLAYER_ATTACK_MULTIPLIER_MIN = 0.5;
const PLAYER_ATTACK_MULTIPLIER_MAX = 1.3;
...
export function computeGwPrediction({ basePPG, position, xG90, xA90, saves90, mppg, starts, chanceOfPlaying, fixture, goalsConceded90, leagueAvgGoalsConceded90 }) {
    ...
    if (fixture.opp !== 'BYE') {
        ...
        breakdown.fdrMultiplier = clamp(rawFdrMultiplier, PLAYER_ATTACK_MULTIPLIER_MIN, PLAYER_ATTACK_MULTIPLIER_MAX);
