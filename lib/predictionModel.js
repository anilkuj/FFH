...
// constant varied): 1.5 -> season pace ~3104; 1.3 -> ~2943; then 1.1 to hit the 2450-2500 target
// (see below). Target (user-specified, aiming for a raw no-captain/no-chips baseline sensibly
// below what a real #1-overall-rank manager's actual season total (2582, WITH captain doubling
// and chips already included) achieves): 2450-2500.
// Raised 1.1 -> 1.2 after the K_ATTACK_OVERALL recalibration (see that constant's comment) exposed
// this ceiling as the actual binding constraint for teams whose fixtures route through the
// overall-strength fallback path (real example: Sunderland vs Ipswich computed a raw 1.2
// multiplier post-recalibration, still fully clamped away at the old MAX=1.1, silently erasing a
// real and legitimate fixture-difficulty signal for exactly the promoted/data-incomplete teams
// this fallback path exists to serve). 1.2 lets that specific real case through without reopening
// the old MAX=1.5 overshoot (season pace ~3104) -- re-verify against real data if raised further.
const PLAYER_ATTACK_MULTIPLIER_MIN = 0.5;
const PLAYER_ATTACK_MULTIPLIER_MAX = 1.2;
