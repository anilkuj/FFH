...
export function recordGwSnapshot(history, { gw, players }) {
    ...
    players.forEach(p => {
        ...
        // Snapshots must be sorted by gw (not insertion order) before capping
        // and before deriving currentTeam/currentPosition. Callers don't always
        // record gameweeks in chronological order in practice: a postponed
        // fixture can mean gw3 is recorded before gw2 gets backfilled once the
        // rescheduled match is finally played. If we trusted insertion order,
        // a late-arriving backfill for an older gw would (a) wrongly evict a
        // more recent snapshot via `.slice(-N)`, and (b) wrongly overwrite
        // currentTeam/currentPosition with stale, older data.
        const updatedSnapshots = [...priorSnapshots, newSnapshot]
            .sort((a, b) => a.gw - b.gw)
            .slice(-MAX_SNAPSHOTS_PER_PLAYER);
        const mostRecentSnapshot = updatedSnapshots[updatedSnapshots.length - 1];

        newPlayers[p.code] = {
            currentTeam: mostRecentSnapshot.team,
            currentPosition: mostRecentSnapshot.position,
